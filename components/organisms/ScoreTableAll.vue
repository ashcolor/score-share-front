  ...
  <div class="container pt-8">
    ...
    <div class="container flex pt-6 bg-base-100 text-center">
      <select class="select select-bordered w-full max-w-xs" v-model="searchSelected">
        <option v-for="searchOption in searchOptions" :value="searchOption.value">{{ searchOption.text }}</option>
      </select>
      <input
        type="text"
      ...
      <div class="form-control flex-row flex-initial w-48">
        <label class="label cursor-pointer mx-auto">
          ...
          <input type="checkbox" class="toggle toggle-primary ml-2" v-model="isColor" checked />
        </label>
      </div>
    </div>
        ...
        <tbody>
          <tr v-for="score in scores?.data.data">
            ...
            <td>
              <div class="avatar">
                <div class="w-12">
                  <img :src="score.score_created_user.twitter_profile_image_url_https" />
                </div>
                <small>
                  {{ score.score_created_user.twitter_screen_name }}
                  <span class="text-muted">@{{ score.score_created_user.twitter_name }}</span>
                </small>
              </div>
            </td>
            <td>
            ...
            <td>
              <a class="pa-2" @click="want(score.id)">
                <Icon icon="ant-design:heart-filled" v-if="score.is_want" color="red" />
                <Icon icon="ant-design:heart" v-else color="red" />
              </a>
            </td>
            <td>
              <a class="pa-2">
                <Icon icon="bi:bookmark-check-fill" v-if="score.is_own" color="green" />
